...
export default function Blog() {
  ...
  return (
    <div
      className="px-[100px] grid justify-center max-sm:px-[20px] bg-[#F5F5F4]"
      dir="rtl"
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 justify-center items-center">
        {data?.hala?.length > 0 && (
          <>
            <div>
              <Image src={getImageSrc(data?.hala[0]?.cover)} alt="" width={100} height={100} className="w-full" />
            </div>
            <div className="flex flex-col gap-4 text-center md:text-right">
              <span className="font-normal text-base text-[#71717A]">
                {formatDate(data?.hala[0]?.createdAt)}
              </span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 my-20">
        {data?.hala?.slice(1).map((blog: any, index: any) => (
          <div key={blog.id} className="flex items-center gap-3">
            <div>
              <Image src={getImageSrc(blog.cover)} alt={blog.title} width={200} height={200} />
            </div>
      ...
      {isLoading ? (
        ...
        <>
          {data &&
            data.categories?.map((blog: any, index: any) => {
              return (
                <div key={index}>
                  <div className="my-20">
                    <div className="">
                      ...
                      {blog.posts?.length > 0 && (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          {blog.posts.map((post: any, index: any) => (
                            <div key={index}>
                              <Link
                                href={`blog/${blog.slug_url}/${post.slug_url}`}
                              >
                                <BlogCard
                                  key={index}
